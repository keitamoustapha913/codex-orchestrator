...
def test_cli_validate_real_codex_smoke_runbook_succeeds_for_dry_run_bundle(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)

    result = _run_cli(["validate-real-codex-smoke-runbook", "--run-dir", str(run_dir)], cwd=tmp_path)

    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["kind"] == "real_codex_smoke_runbook_validation"
    assert payload["valid"] is True


def test_cli_validate_real_codex_smoke_runbook_fails_for_missing_file(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)
...
def test_cli_validate_real_codex_smoke_runbook_outputs_structured_json(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)

    result = _run_cli(["validate-real-codex-smoke-runbook", "--run-dir", str(run_dir)], cwd=tmp_path)

    payload = json.loads(result.stdout)
    assert {"schema_version", "kind", "valid", "run_dir", "validated", "errors", "warnings"}.issubset(payload)


def test_cli_validate_real_codex_smoke_runbook_exit_code_one_for_invalid_bundle(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)
...
def test_cli_validate_real_codex_smoke_runbook_read_only_for_bundle_files(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)
...
def test_cli_validate_real_codex_smoke_runbook_does_not_invoke_codex(tmp_path: Path):
    run_dir = _dry_run_bundle(tmp_path)
    ...
    result = _run_cli(["validate-real-codex-smoke-runbook", "--run-dir", str(run_dir)], cwd=tmp_path, env=env)

    assert result.returncode == 0
    assert not marker.exists()


def test_cli_validate_real_codex_smoke_runbook_reports_missing_run_dir(tmp_path: Path):
    ...
    payload = json.loads(result.stdout)
    assert payload["valid"] is False
    assert payload["errors"][0]["schema"] == "run_dir"
