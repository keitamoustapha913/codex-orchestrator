...
## General Work Decomposition Step

After compilation, run `cxor decomposition --repo <repo>` to inspect the generated work decomposition. The planner creates small bounded work slices, then patchlets with exactly one allowed product/runtime file each. Multiple patchlets may target the same file, and transaction groups follow the dependency graph. See `docs/general_work_decomposition.md`.
