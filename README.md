...
No app.py-specific, app.main-specific, Python-specific, or smoke-prompt regex
parser is supported as the general architecture. `DONE` requires the
model-mediated repo-agnostic path: goal interpretation, provability,
proof-obligation planning, probe planning, mandatory decomposition, independent
proof rerun or validation, goal coverage, and master-prompt satisfaction. See
...
cxor plans general work decomposition before patchlet compilation. The architecture is not one file -> one patchlet; it is one patchlet -> exactly one allowed product/runtime file, and multiple patchlets may target the same file when the work is sequential or proof-heavy. Work slices are derived from impact analysis, proof obligations, dependencies, risk, and task size, then compiled into `patchlet_plan.json`, `dependency_graph.json`, and `transaction_group_plan.json`. Patchlet prompts carry `CODEX_PATCHLET_TIMEOUT_SECONDS` or the 600 second default and avoid memory compacting by staying narrow and self-contained. Inspect with `cxor decomposition --repo <repo>`. See `docs/general_work_decomposition.md` and `docs/multi_patchlet_transaction_graph.md`.
...
## RC6L Positive-Evidence Work Decomposition

Work decomposition assigns patchlet work only when positive planning evidence
links a goal item or proof obligation to a target file. A tracked file,
source-directory placement, or source-like extension is not enough by itself.
An unmatched candidate receives no work, no goal IDs, no proof-obligation IDs,
and no probe IDs.

Support files remain targetable when the frozen master prompt and planning
artifacts explicitly identify them. Each independently provable slice carries
one goal, one proof obligation, and one probe by default, while preserving
legitimate supplemental probes. Multiple patchlets may target one file when
