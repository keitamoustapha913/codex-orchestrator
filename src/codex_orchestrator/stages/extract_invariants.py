...
def extract_invariants(ctx: TargetRepoContext) -> list[dict]:
    ...
    evidence_ids = sorted({eid for node in nodes for eid in node.get("evidence_ids", [])})
    invariant = {
        "schema_version": "1.0",
        "kind": "invariant",
        "invariant_id": "I001",
        "master_goal_id": goal.get("success_goals", [{"goal_id": "G001"}])[0].get("goal_id", "G001"),
        ...
        "runtime_signal_or_condition": "Direct probe observes target behavior deterministically.",
    ...
    write_json(ctx.paths.invariants, [invariant])
