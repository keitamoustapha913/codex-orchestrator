...
def resolve_codex_model_profile(kind: str, env: Mapping[str, str]) -> CodexModelProfile:
    if kind == "patchlet":
        return CodexModelProfile(
            model=(
                env.get("CXOR_PATCHLET_CODEX_MODEL")
                or env.get("CODEX_PATCHLET_MODEL")
                or env.get("CODEX_MODEL")
                or PATCHLET_DEFAULT_MODEL
            ),
            reasoning=(
                env.get("CXOR_PATCHLET_CODEX_REASONING")
                or env.get("CODEX_PATCHLET_REASONING")
                or env.get("CODEX_REASONING")
    ...
    if kind in {"orchestrator", "master_prompt", "group", "transaction", "global", "repair"}:
        return CodexModelProfile(
            model=(
                env.get("CXOR_PLANNING_CODEX_MODEL")
                or env.get("CODEX_ORCHESTRATOR_MODEL")
                or env.get("CODEX_MODEL")
                or ORCHESTRATOR_DEFAULT_MODEL
            ),
            reasoning=(
                env.get("CXOR_PLANNING_CODEX_REASONING")
                or env.get("CODEX_ORCHESTRATOR_REASONING")
                or env.get("CODEX_REASONING")
                or DEFAULT_REASONING
            ),
        )
